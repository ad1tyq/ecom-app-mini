        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <div className="w-full px-4 py-3 rounded-xl bg-apple-bg dark:bg-dark-bg border border-gray-200 dark:border-white/10 flex items-center justify-center">
              <input 
                type="file" 
                accept="image/*" 
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => setImageFile(e.target.files ? e.target.files[0] : null)}
          ...
          <div className="flex items-center justify-between pt-2">
            ...
            <label className="relative inline-flex items-center cursor-pointer">
              <input type="checkbox" name="available" checked={product.available} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setProduct({ ...product, available: e.target.checked })} className="sr-only peer" />
